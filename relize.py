...
boss_kill = BOSSE('boss_anemy.png' , 200, win_height - 700, 400, 500, 8)            


game_lim_time = 25 *1000
...
while run:
    ...
    if not finish:
        ...
        if boss:
            ...
            game_play  =( cur_time - start_time ) / 1000
            window.blit(background, (0, 0))
            boss_kill.reset()
            rocket.update()       
            rocket.reset() 
            ...
            boss_kill.update()
            bullets.update() 
            text_time= font2.render( "Time: "+str(game_play), 1, (100, 255, 255))
            window.blit(text_time, (10, 90))

            text_heal= font2.render( "Heal: "+str(score_boss), 1, (100, 255, 255))
            window.blit(text_heal, (10, 50))
            time.set_timer(USEREVENT, 1000)

            if sprite.spritecollide(boss_kill, bullets, True):
                score_boss -=1
            
            if game_play * 1000 >= game_lim_time:
                window.blit(txt_lose_game, (200, 200))
                finish  = True
            ...
            if not (0 < boss_kill.rect.x < win_width):
                boss_kill.speed *= -1
